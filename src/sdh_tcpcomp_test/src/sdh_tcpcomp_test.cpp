...
extern "C" __declspec( dllexport ) BOOL m_hlclib_conf( struct dsd_hl_clib_dom_conf* ads_conf )
{
    ...
    //-----------------------------------------
    dsc_wsp_helper.m_cb_printf_out( "HEXAMI%03dI %s V%s initialized",
                                    1, SDH_LONGNAME, SDH_VERSION );
    dsc_wsp_helper.m_cb_printf_out( "HEXAMI002I: compiled with %s", dsc_wsp_helper.m_get_version() );
...
extern "C" __declspec( dllexport ) void m_hlclib01( struct dsd_hl_clib_1* ads_trans )
{
    ...
    class  ds_tcpcomp_test*     ads_tcpcomp_test = (ds_tcpcomp_test*)ads_trans->ac_ext;
    ...
    switch ( ads_trans->inc_func ) {
        ...
        case DEF_IFUNC_START:
            ...
            ads_trans->ac_ext = dsc_wsp_helper.m_cb_get_memory( sizeof(ds_tcpcomp_test), true );
            ...
            ads_tcpcomp_test = new(ads_trans->ac_ext) ds_tcpcomp_test();

            // init main working class:
            ads_tcpcomp_test->m_init( &dsc_wsp_helper );

            // setup storage container:
            dsc_wsp_helper.m_use_storage( &(ads_tcpcomp_test->av_storage), SDH_STORAGE_SIZE );
        ...
        case DEF_IFUNC_CLOSE:
            ...
            ads_tcpcomp_test->ds_tcpcomp_test::~ds_tcpcomp_test();
            // clear storage container:
            dsc_wsp_helper.m_no_storage( &(ads_tcpcomp_test->av_storage) );
            // free working class memory:
            dsc_wsp_helper.m_cb_free_memory( (char*)ads_trans->ac_ext, sizeof(ds_tcpcomp_test) );
            break;
