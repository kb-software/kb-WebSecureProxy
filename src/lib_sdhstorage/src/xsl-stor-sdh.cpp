...
*/
//#define D_CHECK_GAP
#define SM_BYPASS_STOR	0
#define SM_TRACK_STOR	0
#define SM_NON_STD_BEHAVIOR_BUGFIX	1
...
#if 1
	return malloc(implen);
#else
	void* achl1;
	BOOL bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                                 DEF_AUX_MEMGET,
                                 &achl1,
                                 implen );  /* size of memory */
   if (bol1 == FALSE) {
...
extern "C" void m_aux_stor_free( struct dsd_stor_sdh_1 *adsp_contr, void *ap_free ) {
	...
	BOOL bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                                 DEF_AUX_MEMFREE,
                                 &ap_free,
                                 sizeof(void *) );  /* size of memory */
   if (bol1 == FALSE) {
      printf( "xslstor1-l%05d-W m_aux_stor_alloc aux DEF_AUX_MEMFREE failed\n",
            __LINE__ );
      return;
   }
#endif
...
#if 1
	return realloc(ap_old, implen);
#else
	void* avol_new = m_aux_stor_alloc(adsp_contr, implen);
	// TODO: memcpy
...
#endif
#ifdef __INSURE__
#define D_AUX_STOR_SIZE (sizeof(struct dsd_aux_stor_header) + sizeof(struct dsd_aux_stor_element))
#else
#define D_AUX_STOR_SIZE (adsp_contr->imc_stor_size)
#endif
#define D_AUX_STOR_LIMIT ((D_AUX_STOR_SIZE-(sizeof(struct dsd_aux_stor_header) + sizeof(struct dsd_aux_stor_element)))/2)

/* start get storage from system                                       */
static void m_aux_stor_start2( struct dsd_stor_sdh_1 *adsp_contr ) {
   ...
   bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                               DEF_AUX_MEMGET,
                               &ap_w1,
                               D_AUX_STOR_SIZE );  /* size of storage element */
   if (bol1 == FALSE) {
...
#ifdef __INSURE__
	adsp_contr->imc_usage_count = 0;
#endif
} /* end m_aux_stor_start()                                            */

static void * m_aux_stor_alloc2( struct dsd_stor_sdh_1 *adsp_contr, int implen ) {
   BOOL       bol1;                         /* working-variable        */
...
#if SM_NON_STD_BEHAVIOR_BUGFIX
	if(implen < 0) {
		printf( "xslstor1-l%05d-W m_aux_stor_alloc invalid length %d\n",
               __LINE__, implen );
      return NULL;
	}
	if(implen == 0)
...
#ifdef __INSURE__
	 adsp_contr->imc_usage_count += implen;
#endif
...
static void m_aux_stor_free2( struct dsd_stor_sdh_1 *adsp_contr, void *ap_free ) {
   unsigned int uml_no_ele;                 /* number of elements      */
   unsigned int uml_end;                    /* end of area             */
   char       *achl1, *achl2;               /* working variables       */
   BOOL       bol1;                         /* working-variable        */
...
#ifdef __INSURE__
		   adsp_contr->imc_usage_count -= (int)*((void **) achl1 + 1);
#endif
		   bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                                       DEF_AUX_MEMFREE,
                                       &achl1,
...
static void* m_aux_stor_realloc2( struct dsd_stor_sdh_1 *adsp_contr, void *ap_old, int implen ) {
   BOOL       bol1;                         /* working-variable        */
   int        iml1;                         /* working variable        */
   unsigned int uml1, uml2, uml3;           /* working-variables       */
   int        iml_len;                      /* length entry            */
...
#if SM_NON_STD_BEHAVIOR_BUGFIX
	if(implen < 0) {
		printf( "xslstor1-l%05d-W m_aux_stor_realloc invalid length %d\n",
               __LINE__, implen );
      return NULL;
	}
	if(implen == 0)
...
#ifdef __INSURE__
		   adsp_contr->imc_usage_count -= (int)*((void **) achl_bs_1 + 1);
#endif
           bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                                       DEF_AUX_MEMFREE,
                                       &achl_bs_1,
   ...
   iml_old_len                              /* length old entry        */
     = ((struct dsd_aux_stor_element *) achl2)->umc_end
         - ((struct dsd_aux_stor_element *) achl2)->umc_start;
   if (implen >= D_AUX_STOR_LIMIT) {   /* acquire big element     */
     bol1 = adsp_contr->amc_aux( adsp_contr->vpc_userfld,
                                 DEF_AUX_MEMGET,
                                 &achl3,
                                 implen + 2 * sizeof(void *) );  /* size of memory */
...
static void m_aux_stor_end2( struct dsd_stor_sdh_1 *adsp_contr ) {
   void       *ap_w1;                       /* working-variable        */
   BOOL       bol1;                         /* working-variable        */
...
extern "C" void m_aux_stor_start( struct dsd_stor_sdh_1 *adsp_contr ) {
	m_aux_stor_start2(adsp_contr);
#if SM_TRACK_STOR
	printf( "xslstor1-l%05d-I m_aux_stor_start adsp_contr=%p\n",
      __LINE__, adsp_contr);
#endif
}

extern "C" void * m_aux_stor_alloc( struct dsd_stor_sdh_1 *adsp_contr, int implen ) {
	void* avol_p = m_aux_stor_alloc2(adsp_contr, implen);
#if SM_TRACK_STOR
	printf( "xslstor1-l%05d-I m_aux_stor_alloc adsp_contr=%p implen=%d avol_p=%p\n",
      __LINE__, adsp_contr, implen, avol_p);
#endif
	return avol_p;
}
...
extern "C" void m_aux_stor_free( struct dsd_stor_sdh_1 *adsp_contr, void *ap_free ) {
#if SM_TRACK_STOR
	printf( "xslstor1-l%05d-I m_aux_stor_free adsp_contr=%p ap_free=%p\n",
      __LINE__, adsp_contr, ap_free);
#endif
	m_aux_stor_free2(adsp_contr, ap_free);
}

extern "C" void * m_aux_stor_realloc( struct dsd_stor_sdh_1 *adsp_contr, void *ap_old, int implen ) {
	void* avol_p = m_aux_stor_realloc2(adsp_contr, ap_old, implen);
#if SM_TRACK_STOR
	printf( "xslstor1-l%05d-I m_aux_stor_realloc adsp_contr=%p ap_old=%p implen=%d avol_p=%p\n",
      __LINE__, adsp_contr, ap_old, implen, avol_p);
#endif
	return avol_p;
}

extern "C" void m_aux_stor_end( struct dsd_stor_sdh_1 *adsp_contr ) {
	m_aux_stor_end2(adsp_contr);
#if SM_TRACK_STOR
	printf( "xslstor1-l%05d-I m_aux_stor_end adsp_contr=%p\n",
      __LINE__, adsp_contr);
#endif
}
#endif
