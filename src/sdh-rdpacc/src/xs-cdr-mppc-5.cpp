...
using hash to last match
#endif
/*+--------------------------------------------------------------------------+*/
/*|                                                                          |*/
/*| PROGRAM NAME: xs-cdr-mppc-5                                              |*/
/*| -------------                                                            |*/
...
#else
#define D_M_CDX_ENC m_cdr_mppc_5_enc
#define D_M_CDX_DEC m_cdr_mppc_5_dec
#endif
...
#define HL_RDP_PACKET_MASK        0X03
#define HL_RDP_PACKET_RDP4        0X00
#define HL_RDP_PACKET_RDP5        0X01
#define HL_RDP_PACKET_RDP60       0X02
...
#define D_HASH_LEN      3                   /* number of characters hash */
#define D_HASH_TYPE     unsigned short int  /* hash C/C++ type         */
#define D_HASH_ENTRIES  0X10000             /* number of hash entries  */
#define D_CRC_INIT      0XFFFF
...
struct dsd_enc_int {                        /* encoding / compression intern */
   ...
   char       *achc_histbu_out;             /* output from history buffer */
   struct dsd_gather_i_1 *adsc_gai1_in_saved;  /* input data saved     */
   ...
   BOOL       boc_histbu_flushed;           /* RDP history buffer has been flushed */
#ifndef B120403
...
static const unsigned char ucrs_leading_bits[256] = {
...
static const unsigned char ucrs_trailing_bits[256] = {
...
static const int imrs_rdp4_co_off_len_total[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
...
static const int imrs_rdp4_co_off_lead_bits[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
   0,                                       /* 1, invalid              */
   3,                                       /* 2                       */
   4,                                       /* 3                       */
...
static const int imrs_rdp4_co_off_add[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
   0,                                       /* 1, invalid              */
   320,                                     /* 2                       */
   64,                                      /* 3                       */
...
static const int imrs_rdp5_co_off_len_total[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
   VAL_BYTE + 1 - 1,                        /* 1, single byte          */
   19,                                      /* 2                       */
   15,                                      /* 3                       */
...
static const int imrs_rdp5_co_off_lead_bits[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
   0,                                       /* 1, invalid              */
   3,                                       /* 2                       */
   4,                                       /* 3                       */
...
static const int imrs_rdp5_co_off_add[ 8 + 1 ] = {
   0,                                       /* zero, invalid           */
   0,                                       /* 1, invalid              */
   2368,                                    /* 2                       */
   320,                                     /* 3                       */
...
static const unsigned short int usrs_crc_table[256] = {
...
#endif
#ifdef DEBUG_130403_01
static int    ims_line_full = 0;
#endif
...
extern "C" void D_M_CDX_ENC( struct dsd_cdr_ctrl *adsp_cdr_ctrl )  /* encode = compression */
{
   ...
   const int  *aimrl_rdp4o5_co_off_len_total;
   const int  *aimrl_rdp4o5_co_off_lead_bits;
   const int  *aimrl_rdp4o5_co_off_add;
// to-do 30.03.13 KB - not needed - start
   unsigned char ucl_w1;                    /* working variable        */
// to-do 30.03.13 KB - not needed - end
   unsigned char ucl_inp_1;                 /* current input first byte */
   unsigned char ucl_inp_2;                 /* current input second byte */
   unsigned char ucl_inp_3;                 /* current input third byte */
#ifdef XYZ1
// to-do 30.03.13 KB - not needed - start
...
// to-do 30.03.13 KB - not needed - end
#endif
   unsigned short int usl_hash_cur;         /* current hash            */
...
#endif
   int        iml_histbu_cur;               /* current position in history buffer */
   int        iml_histbu_match;             /* match position in history buffer */
   int        iml_read_pos;                 /* read characters         */
   int        iml_rdp4o5_co_off_add_max;
   int        iml_len_hist_bu;              /* length history buffer   */
   int        iml_cmp_hist_bu;              /* compare history buffer  */
   ...
   char       *achl_histbu_ref;             /* reference position in history buffer */
   char       *achl_histbu_max;             /* maximum reached position in history buffer */
   char       *achl_histbu_proc;            /* processing history buffer till here */
   char       *achl_histbu_new;             /* new maximum reached position in history buffer */
...
#endif
#ifdef HELP_DEBUG
   D_HASH_TYPE *AUSL_HASH_B;
   char *ACHL_HISTBU_START;
   char *ACHL_CACHE_OFF;
#endif
   ...
   switch (adsp_cdr_ctrl->imc_param_1) {    /* type of compression     */
     ...
     case 60:                               /* RDP6.0                  */
       iml_len_hist_bu = 64 * 1024;
       iml1 = 64 * 1024 + sizeof(imrl_cache_off);
       break;
     default:
#ifdef TRACEHL1
   ...
   bol1 = (*adsp_cdr_ctrl->amc_aux)( adsp_cdr_ctrl->vpc_userfld,  /* get memory */
                                     ...
                                     sizeof(struct dsd_enc_int)
                                       + D_HASH_ENTRIES * sizeof(D_HASH_TYPE)
...
#ifndef HELP_DEBUG
#define AUSL_HASH_B ((D_HASH_TYPE *) (adsl_enc_int + 1))
#define ACHL_HISTBU_START ((char *) (adsl_enc_int + 1) + D_HASH_ENTRIES * sizeof(D_HASH_TYPE))
#define ACHL_CACHE_OFF ((char *) (adsl_enc_int + 1) + D_HASH_ENTRIES * sizeof(D_HASH_TYPE) + 64 * 1024)
#else
   AUSL_HASH_B = (D_HASH_TYPE *) (adsl_enc_int + 1);
   ACHL_HISTBU_START = (char *) (adsl_enc_int + 1) + D_HASH_ENTRIES * sizeof(D_HASH_TYPE);
   ACHL_CACHE_OFF = (char *) (adsl_enc_int + 1) + D_HASH_ENTRIES * sizeof(D_HASH_TYPE) + 64 * 1024;
#endif
   adsl_enc_int->achc_histbu_cur = adsl_enc_int->achc_histbu_max = ACHL_HISTBU_START;
   ...
   memset( ACHL_CACHE_OFF,
           0XFF,
           sizeof(imrl_cache_off) - sizeof(int) );
   *((int *) (ACHL_CACHE_OFF + sizeof(imrl_cache_off) - sizeof(int))) = 0;
   ...
   ACHL_CACHE_OFF = (char *) (adsl_enc_int + 1) + D_HASH_ENTRIES * sizeof(D_HASH_TYPE) + 64 * 1024;
#endif
   if (adsl_enc_int->achc_histbu_out) {     /* output from history buffer */
     goto p_ec_trans_20;                    /* output from history buffer transparent */
   }

   p_ec_trans_40:                           /* output from input       */
   bol_out_huffman = FALSE;                 /* output is not huffman encoded */
   switch (adsp_cdr_ctrl->imc_param_1) {    /* type of compression     */
     case 40:                               /* RDP4                    */
       iml_len_hist_bu = 8192;
       iml_match_max = 8192 - 1;            /* maximum match encoded   */
#ifdef NOT_NEEDED_130401
#ifndef B120403
       adsl_enc_int->chc_comp_header = 0;   /* value for compression header */
#endif
#endif
     ...
     case 60:                               /* RDP6.0                  */
       ...
       memcpy( imrl_cache_off, ACHL_CACHE_OFF, sizeof(imrl_cache_off) );
       break;
     default:
#ifdef TRACEHL1
...
//   }
   ...
   iml_cmp_hist_bu = iml_len_hist_bu - D_HASH_LEN;  /* compare history buffer */
   ...
   achl_histbu_ref = ACHL_HISTBU_START + D_HASH_LEN;  /* reference position in history buffer */
...
#endif
   if (adsl_enc_int->boc_histbu_flushed) {  /* RDP history buffer has been flushed */
     adsl_enc_int->boc_histbu_flushed = FALSE;  /* reset RDP history buffer has been flushed */
     adsp_cdr_ctrl->chrc_header[0] |= HL_RDP_PACKET_FLUSHED;
#ifdef XYZ1
     adsl_enc_int->boc_rdp60_at_front = TRUE;  /* RDP 6.0 packet at front */
#endif
     achl_histbu_cur = adsl_enc_int->achc_histbu_cur = ACHL_HISTBU_START;  /* start of history buffer */
#ifndef B131124
     achl_histbu_max = adsl_enc_int->achc_histbu_max = ACHL_HISTBU_START;
#endif
     achl_histbu_proc = ACHL_HISTBU_START + iml_len_inp;  /* processing history buffer till here */
   }
#ifdef XYZ1
...
#endif
 #ifdef B170116_DD
   if (achl_histbu_proc <= (ACHL_HISTBU_START + iml_len_hist_bu)) {
#else
   if (achl_histbu_proc < (ACHL_HISTBU_START + iml_len_hist_bu)) {
#endif
    goto pecco48;                          /* history buffer has been prepared */
   ...
   if (   (adsp_cdr_ctrl->imc_param_1 != 60)  /* not RDP6.0            */
       || (achl_histbu_cur == ACHL_HISTBU_START)) {  /* current position in history buffer */
     achl_histbu_cur = ACHL_HISTBU_START;   /* start of history buffer */
     achl_histbu_proc = ACHL_HISTBU_START + iml_len_inp;  /* processing history buffer till here */
     goto pecco48;                          /* history buffer has been prepared */
   }
...
#ifdef TRACEHL1
   printf( "ENC l%05d pecco40 RDP6.0 move history buffer at front\n",
           __LINE__ );
#endif
   adsp_cdr_ctrl->chrc_header[0] |= HL_RDP_PACKET_AT_FRONT;
   adsl_enc_int->boc_rdp60_at_front = TRUE;  /* RDP 6.0 packet at front */
   memmove( ACHL_HISTBU_START,
            achl_histbu_cur - 64 * 1024 / 2,
            64 * 1024 / 2 );
   achl_histbu_cur = achl_histbu_max = ACHL_HISTBU_START + 64 * 1024 / 2;  /* start of history buffer */
   achl_histbu_proc = ACHL_HISTBU_START + 64 * 1024 / 2 + iml_len_inp;  /* processing history buffer till here */
   ...
   /* make new entries in table of hashes                             */
   achl_w1 = ACHL_HISTBU_START;             /* start of history buffer */
   achl_w2 = ACHL_HISTBU_START + 64 * 1024 / 2 - D_HASH_LEN + 1;
   while (achl_w1 < achl_w2) {
     usl_hash_cur = (D_CRC_INIT >> 8) ^ usrs_crc_table[ (D_CRC_INIT ^ *((unsigned char *) achl_w1 + 0)) & 0XFF ];
     usl_hash_cur = (usl_hash_cur >> 8) ^ usrs_crc_table[ (usl_hash_cur ^ *((unsigned char *) achl_w1 + 1)) & 0XFF ];
     usl_hash_cur = (usl_hash_cur >> 8) ^ usrs_crc_table[ (usl_hash_cur ^ *((unsigned char *) achl_w1 + 2)) & 0XFF ];
     AUSL_HASH_B[ usl_hash_cur ] = (D_HASH_TYPE) (achl_w1 - ACHL_HISTBU_START);  /* new position in history buffer */
     achl_w1++;                             /* next position history buffer */
   }

   pecco48:                                 /* history buffer has been prepared */
#ifdef TRACEHL1
   iml1 = achl_histbu_cur - ACHL_HISTBU_START;  /* position in history buffer */
...
#endif
#ifdef B160113
   if (achl_histbu_cur == ACHL_HISTBU_START) {  /* is now start of history buffer */
     adsp_cdr_ctrl->chrc_header[0] |= HL_RDP_PACKET_AT_FRONT;
   }
#endif
#ifndef B160113
   ...
   achl_histbu_new = achl_histbu_max;       /* new maximum reached position in history buffer */
   if (achl_histbu_new < achl_histbu_proc) {  /* processing history buffer till here */
     achl_histbu_new = achl_histbu_proc;    /* processing history buffer till here */
   }
   ...
   iml_read_pos = D_HASH_LEN - 1;           /* read characters         */
   ...
   while (achl_inp >= adsl_gai1_in_w1->achc_ginp_end) {
     adsl_gai1_in_w1->achc_ginp_cur = achl_inp;
     adsl_gai1_in_w1 = adsl_gai1_in_w1->adsc_next;
   ...
   ucl_inp_3 = (unsigned char) *achl_inp++;  /* get current input character */
   *achl_histbu_cur++ = ucl_inp_3;          /* insert charater into history buffer */
   if (iml_read_pos != 0) {                 /* read characters         */
     iml_read_pos--;                        /* decrement read characters */
     ucl_inp_1 = ucl_inp_2;                 /* previous characters     */
     ucl_inp_2 = ucl_inp_3;                 /* previous characters     */
     goto p_ec_r4o5_cmp_00;                 /* RDP4/5 new character    */
   }
   usl_hash_cur = (D_CRC_INIT >> 8) ^ usrs_crc_table[ (D_CRC_INIT ^ ucl_inp_1) & 0XFF ];
   usl_hash_cur = (usl_hash_cur >> 8) ^ usrs_crc_table[ (usl_hash_cur ^ ucl_inp_2) & 0XFF ];
   usl_hash_cur = (usl_hash_cur >> 8) ^ usrs_crc_table[ (usl_hash_cur ^ ucl_inp_3) & 0XFF ];
#ifdef TRACEHL1
   printf( "ENC l%05d p_ec_r4o5_cmp_00 pos histbu_cur=%d/0X%X usl_hash_cur=0X%04X.\n",
           __LINE__, achl_histbu_cur - achl_histbu_ref, achl_histbu_cur - achl_histbu_ref, usl_hash_cur );
#endif
   iml_histbu_match = AUSL_HASH_B[ usl_hash_cur ];  /* match position in history buffer */
   AUSL_HASH_B[ usl_hash_cur ] = (D_HASH_TYPE) (achl_histbu_cur - achl_histbu_ref);  /* new position in history buffer */
   if (   (iml_histbu_match <= iml_cmp_hist_bu)
       && (*((unsigned char *) ACHL_HISTBU_START + iml_histbu_match + 0) == ucl_inp_1)
       && (*((unsigned char *) ACHL_HISTBU_START + iml_histbu_match + 1) == ucl_inp_2)
       && (*((unsigned char *) ACHL_HISTBU_START + iml_histbu_match + 2) == ucl_inp_3)
#ifndef TRY_130408_01
#ifdef TRY_130403_01
       && (iml_histbu_match != (achl_histbu_cur - achl_histbu_ref))
#endif
#else
       && (   (iml_histbu_match < (achl_histbu_cur - achl_histbu_ref))
           || (iml_histbu_match > ((achl_histbu_cur - achl_histbu_ref) + 2)))
#endif
       && (   ((achl_histbu_ref + iml_histbu_match) <= achl_histbu_max)  /* maximum reached position in history buffer */
           || ((achl_histbu_ref + iml_histbu_match) <= achl_histbu_cur))) {  /* current position in history buffer */
     goto p_ec_r4o5_cmp_20;                 /* RDP456 match found      */
   }
   if (bol_out_huffman == FALSE) {          /* output is not huffman encoded */
     if (((signed char) ucl_inp_1) >= 0) {
       iml_shift_v <<= VAL_BYTE;            /* shift old bits          */
       iml_shift_v |= ucl_inp_1;            /* get new bits            */
       iml_shift_c += VAL_BYTE;             /* shift-count             */
     } else {
       iml_shift_v <<= VAL_BYTE + 1;        /* shift old bits          */
       iml_shift_v |= 0X0100 | (ucl_inp_1 & 0X7F);  /* get new bits    */
     ...
     iml_shift_v |= usrs_rdp60_encode_2[ ucl_inp_1 ] << iml_shift_c;  /* get new bits */
     iml_shift_c += ucrs_rdp60_encode_1[ ucl_inp_1 ];  /* shift-count  */
     while (iml_shift_c >= VAL_BYTE) {      /* shift-count at least byte */
       iml_len_out++;                       /* increment length of output */
       if (iml_len_out >= iml_len_inp) {    /* compressed is longer than original */
#ifdef DEBUG_130403_01
   ...
   p_ec_r4o5_cmp_20:                        /* RDP456 match found      */
   iml_copy_pos = (achl_histbu_cur - achl_histbu_ref) - iml_histbu_match;  /* copy position */
   achl_w1 = achl_histbu_ref + iml_histbu_match;  /* compare here in history buffer */
   achl_w2 = achl_w1 - D_HASH_LEN + iml_match_max;  /* longest than can be encoded */
   if (achl_w2 > achl_histbu_new) {         /* new maximum reached position in history buffer */
     achl_w2 = achl_histbu_new;             /* new maximum reached position in history buffer */
   }

   p_ec_r4o5_cmp_24:                        /* compare direct with history buffer */
   if (achl_w1 >= achl_w2) {                /* longest encoding or end of filled history buffer */
     goto p_ec_r4o5_cmp_40;                 /* end of match found      */
   }
   while (achl_inp >= adsl_gai1_in_w1->achc_ginp_end) {
     adsl_gai1_in_w1->achc_ginp_cur = achl_inp;
     adsl_gai1_in_w1 = adsl_gai1_in_w1->adsc_next;
   ...
   if (*achl_inp != *achl_w1) {             /* characters do not match */
     goto p_ec_r4o5_cmp_40;                 /* end of match found      */
   }
   *achl_histbu_cur++ = *achl_inp++;        /* insert charater into history buffer */
   achl_w1++;                               /* next position history buffer */
   goto p_ec_r4o5_cmp_24;                   /* compare direct with history buffer */

   p_ec_r4o5_cmp_40:                        /* end of match found      */
   iml_cmp_max = achl_w1 - (ACHL_HISTBU_START + iml_histbu_match);  /* longest match */
   /* output match                                                     */
   iml1 = iml_copy_pos;
   ...
   printf( "ENC l%05d copy-offset=%d/0X%p len-of-match=%d/0X%p pos-histbu-cur=%d/0X%p.\n",
           __LINE__,
           iml1, iml1,
           iml_cmp_max, iml_cmp_max,
           achl_histbu_cur - ACHL_HISTBU_START - iml_cmp_max,
           achl_histbu_cur - ACHL_HISTBU_START - iml_cmp_max );
#endif
...
#endif
   iml1 = iml_cmp_max;                      /* longest match exactly   */
...
//   iml_shift_c -= VAL_BYTE;               /* shift-count             */
   ...
   /* end output match RDP4 + RDP5                                     */
   iml_read_pos = D_HASH_LEN - 1;           /* read characters         */
   if (adsl_gai1_in_w1) {                   /* not yet end of input reached */
     goto p_ec_r4o5_cmp_00;                 /* RDP456 new character    */
   }
   goto p_ec_r4o5_ret_00;                   /* RDP456 all processed    */
   ...
   /* the length is in iml_cmp_max                                     */
   iml1 = 0;                                /* index of table          */
   do {                                     /* loop over table entries */
     if (iml_cmp_max < usrs_rdp60_length_1[ iml1 + 1 ]) break;
     iml1++;                                /* next index              */
   } while (iml1 < (sizeof(usrs_rdp60_length_1) / sizeof(usrs_rdp60_length_1[0]) - 1));
   iml_shift_v |= usrs_rdp60_length_4[ iml1 ] << iml_shift_c;  /* get new bits */
   iml_shift_c += ucrs_rdp60_length_3[ iml1 ];  /* shift-count add number of bits */
   iml2 = ucrs_rdp60_length_2[ iml1 ];      /* get extra number of bits */
   if (iml2 > 0) {                          /* we need extra bits      */
     iml_shift_v |= (iml_cmp_max - usrs_rdp60_length_1[ iml1 ]) << iml_shift_c;  /* get new bits */
     iml_shift_c += iml2;                   /* shift-count add number of bits */
   }
#ifdef DEBUG_110803_01                      /* RDP6.0 display length-of-match */
   printf( "ENC l%05d length-of-match iml_cmp_max=0X%04X iml1=0X%04X iml2=0X%04X usrs_rdp60_length_1[ iml1 ]=0X%04X iml_shift_v=0X%08X.\n",
           __LINE__, iml_cmp_max, iml1, iml2 usrs_rdp60_length_1[ iml1 ], (unsigned int) iml_shift_v );
#endif
   while (iml_shift_c >= VAL_BYTE) {        /* shift-count at least byte */
     iml_len_out++;                         /* increment length of output */
     if (iml_len_out >= iml_len_inp) {      /* compressed is longer than original */
   ...
   iml1 = iml_read_pos = D_HASH_LEN - 1 - iml_read_pos;  /* read characters */
   if (iml1 == 0) {                         /* no more output characters */
     goto p_ec_r4o5_ret_20;                 /* output remaining bits   */
   }
   if (iml1 > 1) {                          /* more than one character */
     if (bol_out_huffman == FALSE) {        /* output is not huffman encoded */
       if (((signed char) ucl_inp_1) >= 0) {
         iml_shift_v <<= VAL_BYTE;          /* shift old bits          */
         iml_shift_v |= ucl_inp_1;          /* get new bits            */
   ...
   if (bol_out_huffman == FALSE) {          /* output is not huffman encoded */
     if (((signed char) ucl_inp_2) >= 0) {
       iml_shift_v <<= VAL_BYTE;            /* shift old bits          */
       iml_shift_v |= ucl_inp_2;            /* get new bits            */
       iml_shift_c += VAL_BYTE;             /* shift-count             */
     } else {
       iml_shift_v <<= VAL_BYTE + 1;        /* shift old bits          */
       iml_shift_v |= 0X0100 | (ucl_inp_2 & 0X7F);  /* get new bits    */
       iml_shift_c += VAL_BYTE + 1;         /* shift-count             */
     }
     while (iml_shift_c >= VAL_BYTE) {      /* shift-count at least byte */
       iml_len_out++;                       /* increment length of output */
     ...
     iml_shift_v |= usrs_rdp60_encode_2[ ucl_inp_2 ] << iml_shift_c;  /* get new bits */
     iml_shift_c += ucrs_rdp60_encode_1[ ucl_inp_2 ];  /* shift-count  */
     while (iml_shift_c >= VAL_BYTE) {      /* shift-count at least byte */
       iml_len_out++;                       /* increment length of output */
       if (iml_len_out >= iml_len_inp) {    /* compressed is longer than original */
#ifdef DEBUG_130403_01
   ...
   p_ec_r4o5_ret_20:                        /* output remaining bits   */
   if (bol_out_huffman == FALSE) {          /* output is not huffman encoded */
     ...
     iml_shift_c += ucrs_rdp60_encode_1[ VAL_TAB_R60_D1_END ];  /* shift-count */
     while (iml_shift_c > 0) {              /* output all bits         */
       iml_len_out++;                       /* increment length of output */
       if (iml_len_out >= iml_len_inp) {    /* compressed is longer than original */
#ifdef DEBUG_130403_01
     ...
     /* copy RDP6.0 cache offset                                       */
     memcpy( ACHL_CACHE_OFF, imrl_cache_off, sizeof(imrl_cache_off) );
   }
   adsl_enc_int->boc_rdp60_at_front = FALSE;  /* not RDP 6.0 packet at front */
   adsl_enc_int->achc_histbu_cur = achl_histbu_cur;  /* current position in history buffer */
   adsl_enc_int->achc_histbu_max = achl_histbu_new;  /* new maximum reached position in history buffer */
   ...
   adsl_enc_int->boc_histbu_flushed = TRUE;  /* RDP history buffer has been flushed */
   adsl_enc_int->boc_rdp60_at_front = FALSE;  /* not RDP 6.0 packet at front */
   /* history buffer has already filled with input, this is first output */
   if (adsl_gai1_in_w1) {                   /* still gather input      */
     adsl_gai1_in_w1->achc_ginp_cur = achl_inp;
     while (achl_inp >= adsl_gai1_in_w1->achc_ginp_end) {
       adsl_gai1_in_w1->achc_ginp_cur = achl_inp;
       adsl_gai1_in_w1 = adsl_gai1_in_w1->adsc_next;
       if (adsl_gai1_in_w1 == NULL) {       /* end of input reached    */
         break;                             /* RDP5 all processed      */
       }
       achl_inp = adsl_gai1_in_w1->achc_ginp_cur;  /* current input      */
     }
   }
   adsl_enc_int->adsc_gai1_out_saved = adsl_gai1_in_w1;
   achl_w1 = adsl_enc_int->achc_histbu_cur;  /* get position in history buffer before last encode */
   if (achl_w1 >= achl_histbu_cur) {        /* current position in history buffer */
     goto p_ec_trans_40;                    /* output from input       */
   }
   adsl_enc_int->achc_histbu_cur = achl_histbu_cur;  /* current position in history buffer */
   adsl_enc_int->achc_histbu_out = achl_w1;  /* output from history buffer */

   p_ec_trans_20:                           /* output from history buffer transparent */
   iml1 = adsp_cdr_ctrl->achc_out_end - adsp_cdr_ctrl->achc_out_cur;
   if (iml1 <= 0) return;                   /* no space in output area */
   achl_w1 = adsl_enc_int->achc_histbu_out;  /* output from history buffer */
   achl_w2 = adsl_enc_int->achc_histbu_cur;  /* current position in history buffer */
   iml2 = achl_w2 - achl_w1;                /* copy from history buffer */
   if (iml1 > iml2) iml1 = iml2;            /* this part to copy       */
   memcpy( adsp_cdr_ctrl->achc_out_cur,
           achl_w1,
           iml1 );
   adsp_cdr_ctrl->achc_out_cur += iml1;
   achl_w1 += iml1;
   if (achl_w1 < achl_w2) {                 /* more in history buffer  */
     adsl_enc_int->achc_histbu_out = achl_w1;  /* output from history buffer */
     return;                                /* wait for more space in output area */
   }
   adsl_enc_int->achc_histbu_out = NULL;    /* no more output from history buffer */
   if (adsl_enc_int->adsc_gai1_out_saved) {
     goto p_ecco_out_20;                    /* pass saved output data  */
   }
   adsp_cdr_ctrl->boc_sr_flush = TRUE;      /* at end of data          */
   return;                                  /* all done                */
...
#ifndef HELP_DEBUG
#undef AUSL_HASH_B
#undef ACHL_HISTBU_START
#undef ACHL_CACHE_OFF
#endif
...
#define IMRL_CACHE_OFF ((int *) ((char *) (adsl_dec_int + 1) + 64 * 1024))
   ...
   *(IMRL_CACHE_OFF + VAL_CACHE_OFF) = 0;
...
#endif
   if (adsp_cdr_ctrl->imc_func == DEF_IFUNC_START) {
     adsp_cdr_ctrl->ac_ext = NULL;          /* store address of fields */
     adsp_cdr_ctrl->imc_len_header = 1;       /* length of header data   */
     adsp_cdr_ctrl->boc_compressed = TRUE;    /* always compressed       */
     adsp_cdr_ctrl->imc_save_mp_needed = 0;  /* do not use save-area mp */
     adsp_cdr_ctrl->imc_func = DEF_IFUNC_CONT;  /* next call continue  */
   ...
   if (adsp_cdr_ctrl->ac_ext == NULL) return;  /* no memory acquired before */
   bol1 = (*adsp_cdr_ctrl->amc_aux)( adsp_cdr_ctrl->vpc_userfld,
                                     DEF_AUX_MEMFREE,
                                     &adsp_cdr_ctrl->ac_ext,
   ...
   if (adsl_dec_int) goto pdcfc20;          /* all set before          */
   /* function start                                                   */
   switch (adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK) {  /* type of decompression */
     case HL_RDP_PACKET_RDP4:               /* RDP4                    */
       iml1 = iml2 = 8192;
       break;
     case HL_RDP_PACKET_RDP5:               /* RDP5                    */
       iml1 = iml2 = 64 * 1024;
       break;
     case HL_RDP_PACKET_RDP60:              /* RDP6.0                  */
       iml1 = 64 * 1024 + (VAL_CACHE_OFF + 1) * sizeof(int);
       iml2 = 64 * 1024;
       break;
     default:
#ifdef TRACEHL1
       printf( "DEC l%05d start header 0X%02X invalid value / allowed: 40 50 60.\n",
               __LINE__, (unsigned char) (adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK) );
#endif
       adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error   */
       return;                              /* return to main-prog     */
   }
   ...
   adsl_dec_int->achc_histbu_cur = (char *) (adsl_dec_int + 1);  /* current position in history buffer */
   adsl_dec_int->achc_histbu_max = (char *) (adsl_dec_int + 1);  /* current position in history buffer */
   adsl_dec_int->chc_comp_header = adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK;  /* value for compression header */
   adsp_cdr_ctrl->ac_ext = adsl_dec_int;    /* store address of fields */
   if ((adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK) != HL_RDP_PACKET_RDP60) {  /* not RDP6.0 */
     goto pdcfc40;                            /* start decompression     */
   }
   /* prepare RDP6.0 cache offset                                      */
   memset( IMRL_CACHE_OFF,
           0XFF,
   ...
   *(IMRL_CACHE_OFF + VAL_CACHE_OFF) = 0;
   ...
   pdcfc20:                                 /* all set before          */
   if ((adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK) != adsl_dec_int->chc_comp_header) {  /* not as initialized */
#ifdef TRACEHL1
     printf( "DEC l%05d start header 0X%02X invalid value / set before: 0X%02X.\n",
             __LINE__, (unsigned char) (adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_MASK), adsl_dec_int->chc_comp_header );
#endif
     adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error     */
     return;                                /* return to main-prog     */
   }

   pdcfc40:                                 /* start decompression     */
#ifdef TRACEHL1
   {
     char *achh_w1 = NULL;
...
#endif
       && (adsl_dec_int->chc_comp_header == HL_RDP_PACKET_RDP60)
       && ((adsp_cdr_ctrl->chrc_header[0] & HL_RDP_PACKET_FLUSHED) == 0)
...
#endif
#endif
   switch (adsl_dec_int->chc_comp_header) {  /* type of decompression  */
     case HL_RDP_PACKET_RDP4:               /* RDP4                    */
       goto p_dcco_r4_00;                   /* get codeword RDP4       */
     case HL_RDP_PACKET_RDP5:               /* RDP5                    */
       goto p_dcco_r5_00;                   /* get codeword RDP5       */
...
#ifdef TRACEHL1
   printf( "DEC l%05d pdcbu00 header 0X%02X invalid.\n",
           __LINE__, adsl_dec_int->chc_comp_header );
#endif
   adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error       */
   return;                                  /* return to main-prog     */
...
#ifdef TRACEHL1
   printf( "DEC l%05d pdcbu20 header 0X%02X invalid.\n",
           __LINE__, adsl_dec_int->chc_comp_header );
#endif
   adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error       */
   return;                                  /* return to main-prog     */
...
#ifdef TRACEHL1
   printf( "DEC l%05d pdcco64 header 0X%02X invalid.\n",
           __LINE__, adsl_dec_int->chc_comp_header );
#endif
   adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error       */
   return;                                  /* return to main-prog     */
...
#ifdef TRACEHL1
   printf( "DEC l%05d pdcco72 header 0X%02X invalid.\n",
           __LINE__, adsl_dec_int->chc_comp_header );
#endif
   adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error       */
   return;                                  /* return to main-prog     */
...
#ifdef TRACEHL1
   printf( "DEC l%05d pdcco80 header 0X%02X invalid.\n",
           __LINE__, adsl_dec_int->chc_comp_header );
#endif
   adsp_cdr_ctrl->imc_return = DEF_IRET_ERRAU;  /* message error       */
   return;                                  /* return to main-prog     */
