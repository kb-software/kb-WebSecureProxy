...
#endif
#else
#define D_NTLM_AUTH_FLAGS_NORMAL (0XA2888205 | NTLMSSP_NEGOTIATE_KEY_EXCH)
...
//#define D_NTLM_AUTH_FLAGS_NORMAL ((0XA2888205 & ~(NTLMSSP_NEGOTIATE_VERSION|NTLMSSP_NEGOTIATE_OEM|NTLMSSP_NEGOTIATE_TARGET_INFO)) | NTLMSSP_NEGOTIATE_KEY_EXCH | NTLMSSP_NEGOTIATE_LM_KEY | NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL | NTLMSSP_TARGET_TYPE_SERVER)
#define D_NTLM_AUTH_FLAGS_NORMAL (0XA2888205 | NTLMSSP_NEGOTIATE_KEY_EXCH | NTLMSSP_NEGOTIATE_LM_KEY | NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL | NTLMSSP_TARGET_TYPE_SERVER)

//0x40000000 NTLMSSP_NEGOTIATE_KEY_EXCH  -> JWT
//		0x02000000 NTLMSSP_NEGOTIATE_VERSION   -> KB
//		0x00800000 NTLMSSP_NEGOTIATE_TARGET_INFO -> KB
//		0x00020000 NTLMSSP_TARGET_TYPE_SERVER  -> JWT
//		0x00000080 NTLMSSP_NEGOTIATE_LM_KEY    -> JWT
//		0x00000040 NTLMSSP_NEGOTIATE_DATAGRAM  -> JWT
//		0x00000002 NTLMSSP_NEGOTIATE_OEM       -> JWT
...
#define ACHL_SERVER_HANDLE (byrl_work1 + 32 + 16 + 16 + 16 + 16 + 16 + RC4_STATE_SIZE)
	 ...
	 if (adsp_ntlm_req->achc_ntlm_client_sign_key) {
		memcpy( adsp_ntlm_req->achc_ntlm_client_sign_key, ACHL_CLIENT_SIGNKEY, LEN_NTLM_SIGN_KEY );
     }
	 if (adsp_ntlm_req->achc_ntlm_client_seal_key) {
		memcpy( adsp_ntlm_req->achc_ntlm_client_seal_key, ACHL_CLIENT_SEALKEY, LEN_NTLM_SEAL_KEY );
     }
	 if (adsp_ntlm_req->achc_ntlm_server_sign_key) {
		memcpy( adsp_ntlm_req->achc_ntlm_server_sign_key, ACHL_SERVER_SIGNKEY, LEN_NTLM_SIGN_KEY );
     }
	 if (adsp_ntlm_req->achc_ntlm_server_seal_key) {
		memcpy( adsp_ntlm_req->achc_ntlm_server_seal_key, ACHL_SERVER_SEALKEY, LEN_NTLM_SEAL_KEY );
     }
   }
   adsp_ntlm_req->umc_message_flags = uml_challenge_message_flags;
   memset( byrl_hmac_1, 0X36, sizeof(byrl_hmac_1) );  /* for HMAC      */
   memset( byrl_hmac_2, 0X5C, sizeof(byrl_hmac_2) );  /* for HMAC      */
   iml1 = 0;                                /* clear index             */
   do {
